...
export class WorktreeManager {
   ...
   *
   * Dependency installation has been hoisted out of this method into
   * `prepareTaskWorkspace` (in `src/runtime/workspace-prepare.ts`) so the
   * git surface stays focused on git operations. Callers must invoke
   * `prepareTaskWorkspace(ref.path)` before they create the per-task
   * Workspace handle.
   */
