...
**Timing note.** For operations that land as `active` immediately (`edit`, `promote`, `demote`, `retire`), lessons are retired at proposal time. For operations that land as `proposed` pending approval (`fork`, `merge`), retirement is deferred — lessons stay active until the approval endpoint runs. List them anyway; the system records your intent.
