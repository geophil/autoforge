...
Canonical roadmap: `docs/qmd/model-cost-efficiency-v2.md`.

## Implementation Status

PR 1 through PR 8 are implemented locally as the V2 foundation. PR 9 keeps the QMD roadmap canonical and preserves this file as the PR-sized development plan. Adaptive routing, semantic retrieval, summarization policy, dashboards, and cost caps remain post-foundation backlog.

## PR Units

- PR 1: Runtime Controls Extraction (complete)
...
- PR 2: MCP/QMD Adapter Extraction (complete)
...
- PR 3: Unified Tool Execution Outcome (complete)
...
- PR 4: Prompt Envelope Foundation (complete)
...
- PR 5: Provider Cache Adapter Discipline (complete)
...
- PR 6: Runtime Telemetry Summary Builder (complete)
...
- PR 7: Durable Dispatch Telemetry Events (complete)
...
- PR 8: Reporting/KPI Query Helpers (complete)
...
- PR 9: Canonical Roadmap Documentation (current)
  - Maintain `docs/qmd/model-cost-efficiency-v2.md` as the canonical roadmap.
...
- `bun run lint` passes.
- Focused runtime, provider, telemetry, routing, and metrics tests pass.
- `bun test tests/unit` passes.
- `bun test tests/integration/plan-review.test.ts tests/integration/awaiting-intervention.test.ts` currently has the known local checkpoint/worktree blocker: plan-review passes, while five awaiting-intervention rollback/checkpoint assertions fail after `git worktree add` falls back to a plain directory.
