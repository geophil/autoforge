...
if (qmdContext?.status === "used" && hasQueriesOrDocuments(qmdContext)) {
  return { status: "used" };
}
if (qmdContext?.status === "fallback" && hasAuditableFallbackEvidence(qmdContext, observedQmdTools)) {
  return { status: "degraded", failureCategory: "qmd_degraded" };
}
this.pauseForIntervention({
  failureCategory: observedQmdTools.length > 0 ? "qmd_unavailable" : "planner_ignored_qmd",
  failureReason: "planner output missing required QMD knowledgebase evidence"
});
```

**Implemented in**: `OrchestratorService.assessQmdEvidenceForPlanner()`.
...
11. **Execution contract**: Before coder work, the orchestrator emits `execution_contract` with WIP limit 1, validation hierarchy, and each subtask's behavior/scope/verification/evidence contract. The same computed plan-contract validator is used by plan review, approval, repair, and execution. Safe aliases such as `description -> behavior` are marked as repairs; missing verification/test/evidence remains invalid. STANDARD/THOROUGH tasks pause to `awaiting_intervention` with `failure_category=planner_contract_invalid` if unrepaired required fields remain.
...
- Execution contract invalid for STANDARD/THOROUGH → `awaiting_intervention` with `failure_category=planner_contract_invalid`; deterministic alias repairs can be applied with `POST /api/tasks/:id/repair-plan-contract`
- Planner ignored QMD evidence while `QMD_MCP_URL` is configured → task transitions to `awaiting_intervention` with `failure_category=planner_ignored_qmd`
- Planner attempted QMD but produced no usable or degraded fallback evidence → task transitions to `awaiting_intervention` with `failure_category=qmd_unavailable`
- Planner produced degraded but auditable QMD fallback evidence → planning proceeds with `done_with_concerns`
